...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  int _reelsBlocked = 0;
  int _remainingLives = 4;
  bool _isPostModeActive = false;
  int _postModeRemainingSec = 0;
  ...
  Future<void> _refreshStats() async {
    ...
    final postMode = await NativeShieldService.isPostModeActive();
    final postSec = await NativeShieldService.getPostModeRemainingSeconds();
    if (mounted) {
      final s = stats['totalReelsScrolled'] ?? 0;
      final b = stats['totalReelsBlocked'] ?? 0;
      if (s != _reelsScrolled ||
          b != _reelsBlocked ||
          lives != _remainingLives ||
          postMode != _isPostModeActive ||
          postSec != _postModeRemainingSec) {
        setState(() {
          _reelsScrolled = s;
          _reelsBlocked = b;
          _remainingLives = lives;
          _isPostModeActive = postMode;
          _postModeRemainingSec = postSec;
        });
      }
    }
  }
  ...
  Widget _buildTopBar(BuildContext context) {
    return NeoTopBar(appState: widget.appState);
  }
